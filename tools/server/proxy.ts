...
const proxyNotConfigured = (target: any) => {
    return `Proxy ${target} does not exist. Please make sure the proxy is configured in tools/config/stub_proxy/proxy_address.json`;
};
    ...
    return startServer(app, options, 'proxy').tap((server: any) => {
        return server.on('upgrade', (req: any, socket: any, head: any) => {
            ...
            return upstream.ws(req, socket, head, {
                ...
                log('Please verify `tools/config/stub_proxy/proxy-address.json` is correct:');
